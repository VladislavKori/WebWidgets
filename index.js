const {
  app,
  BrowserWindow,
  ipcMain,
  ipcRenderer,
  Tray,
  nativeImage,
  Menu,
} = require("electron");
const fs = require("fs");

let globalWindow;
let widgetWindwos = [];

const createMainWindow = () => {
  globalWindow = new BrowserWindow({
    width: 600,
    height: 400,
    webPreferences: {
      nodeIntegration: true,
      contextIsolation: false,
    },
  });

  globalWindow.loadFile("./index.html");
};

function createWidgetWindow() {
  const widgetWindow = new BrowserWindow({
    width: 155,
    height: 155,
    transparent: true,
    frame: false,
    type: "toolbar",
    resizable: false,
    titleBarStyle: "hidden",
    webPreferences: {
      nodeIntegration: true,
      contextIsolation: false,
    },
  });

  widgetWindwos.push(widgetWindow);

  widgetWindow.on("blur", () => {
    widgetWindow.setBackgroundColor("#00000000");
  });

  widgetWindow.on("focus", () => {
    widgetWindow.setBackgroundColor("#00000000");
  });

  widgetWindow.loadFile("./AppleTV/index.html");

  widgetWindow.setAlwaysOnTop(false, "modal-panel", 0);
}

function createCustomWidget({ name, width, hieght, code }) {
  const widgetWindow = new BrowserWindow({
    width: width,
    height: hieght,
    transparent: true,
    frame: false,
    type: "toolbar",
    resizable: false,
    titleBarStyle: "hidden",
    webPreferences: {
      nodeIntegration: true,
      contextIsolation: false,
    },
  });

  widgetWindwos.push(widgetWindow);

  widgetWindow.on("blur", () => {
    widgetWindow.setBackgroundColor("#00000000");
  });

  widgetWindow.on("focus", () => {
    widgetWindow.setBackgroundColor("#00000000");
  });

  const WidgetSourceFolder = __dirname + "/GeneratedWidget/" + name;

  try {
    if (!fs.existsSync(WidgetSourceFolder)) {
      fs.mkdirSync(WidgetSourceFolder);
    }
  } catch (e) {
    console.error(e);
  }

  fs.writeFile(WidgetSourceFolder + "/index.html", code, (err) => {
    if (err) {
      console.error(err);
    }
    widgetWindow.loadFile(WidgetSourceFolder + "/index.html");
  });

  widgetWindow.setAlwaysOnTop(false, "modal-panel", 0);
}

function lockWidgets() {
  widgetWindwos.map((item) => {
    item.setIgnoreMouseEvents(true);
  });
}

function unlockWidgets() {
  widgetWindwos.map((item) => {
    item.setIgnoreMouseEvents(false);
  });
}

ipcMain.on("createWidget", (event, args) => {
  const { code, width, hieght, name } = JSON.parse(args);
  createCustomWidget({ name, code, width, hieght });
});

ipcMain.on("openWidget", (event, args) => {
  createWidgetWindow();
});

ipcMain.on("lockWidgets", (event, args) => {
  lockWidgets();
});

ipcMain.on("unlockWidgets", (event, args) => {
  unlockWidgets();
});

const createTray = () => {
  const icon = nativeImage.createFromDataURL(
    "data:image/png;base64,"
  );
  const tray = new Tray(icon);

  const contextMenu = Menu.buildFromTemplate([
    {
      label: "Show App",
      click: () => {
        createWindow();
      },
    },
    {
      label: "Quit",
      click: () => {
        app.quit();
      },
    },
  ]);

  tray.setToolTip("This is my application.");
  tray.setContextMenu(contextMenu);
};

app.whenReady().then(() => {
  createTray();
  createMainWindow();
});
